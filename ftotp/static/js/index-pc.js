    ...
    $STAR = {
            ...
            $selectFather.on('mouseover',function (event) {
                ...
                switch (type){
                    case 'android':
                        ...
                        break;
                    case 'android-concept':
                        $deviceTypeImg.find('img.android-concept').fadeIn().siblings().hide();
                        break;
                    case 'iPhone':
                        $deviceTypeImg.find('img.iPhone').fadeIn().siblings().hide();
                        break;
                    case 'iPhone-concept':
                        $deviceTypeImg.find('img.iPhone-concept').fadeIn().siblings().hide();
                        break;
                    case 'iPad':
        ...
        selectInfo: function(){
            var that = this,
                ...
                chooseContent;
            chooseContent = function($Obj,$content,androidShow,iPhoneShow,iPadShow,UWPShow,WPShow){
            ...
            $selectFather.on('click',function(event){
                ...
                switch (type){
                    case 'android':
                        chooseContent($curObj,that.$androidC,true,false,false,false,false);
                        break;
                    case 'iPhone':
                        chooseContent($curObj,that.$iPhoneC,false,true,false,false,false);
                        break;
                    case 'iPad':
                        chooseContent($curObj,that.$iPadC,false,false,true,false,false);
                        break;
                    case 'UWP':
                        chooseContent($curObj,that.$UWPC,false,false,false,true,false);
                        break;
                    case 'WP':
                        chooseContent($curObj,that.$WPC,false,false,false,false,true);
                        break;
                }
            });
            return this;
            ...
            $.each(list, function(index, value) {
                if( index < loopNum){
                    ...
                    that.info += '<div class="element">' +
                        ...
                        '<div class="text-wrap">'+
                        '<p class="info-title">OTPMobile<span>'+ type +'</span>' + infoVersion +'</p>';
        ...
        // init:function(){
            // this.selectDeviceType().selectInfo().scrollLoad()
                ...
                // .showContent("//app.bilibili.com/x/v2/version?mobi_app=wphone",'UWP');
        // }
		init:function(){
            this.selectDeviceType().selectInfo().scrollLoad()
                .showContent("",'android')
